...
################################################################################
  ...
  def batch_update(self,index):
    ...
    # temp_W = self.W
    self.W = W_batch_update - np.multiply(self.eta*self.decay_factor, self.W) #L2 regularisation
...
for epoch in range(FFN.epoch_num):
...
accuracy = sum([1 for i,j in zip(test_label,Y_test) if i==j])/len(Y_test)
